package com.ld.protean.service
...
import com.ld.protean.ext.enableAGPS
import com.ld.protean.ext.enableGetFromLocation
import com.ld.protean.ext.enableNMEA
import com.ld.protean.ext.enableRequestGeofence
...
object MockServiceHelper {
    const val PROVIDER_NAME = "protean"
    ...
    /**
     * 反定位拉回：通知模块开启/关闭"持续重播模拟位置"的循环
     */
    fun setLoopBroadcastLocation(locationManager: LocationManager, enable: Boolean): Boolean {
        if (!::randomKey.isInitialized) {
            return false
        }
        val rely = Bundle()
        rely.putString("command_id", "set_loop_broadcast_location")
        rely.putBoolean("loop_broadcast_location", enable)
        return locationManager.sendExtraCommand(PROVIDER_NAME, randomKey, rely)
    }

    fun setBearing(locationManager: LocationManager, bearing: Double): Boolean {        if (!::randomKey.isInitialized) {
            return false
        }
        val rely = Bundle()
    ...
    fun putConfig(locationManager: LocationManager, context: Context): Boolean {
        ...
        FakeLoc.enableMockWifi = context.disableWifiScan
