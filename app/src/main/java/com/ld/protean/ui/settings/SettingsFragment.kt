package com.ld.protean.ui.settings
...
import com.ld.protean.databinding.FragmentSettingsBinding
import com.ld.protean.ext.accuracy
import com.ld.protean.ext.altitude
import com.ld.protean.ext.debug
import com.ld.protean.ext.disableFusedProvider
import com.ld.protean.ext.disableGetCurrentLocation
import com.ld.protean.ext.disableRegisterLocationListener
import com.ld.protean.ext.disableWifiScan
import com.ld.protean.ext.hookSensor
import com.ld.protean.ext.mapTileProvider
import com.ld.protean.ext.minSatelliteCount
import com.ld.protean.ext.loopBroadcastLocation
import com.ld.protean.ext.needDowngradeToCdma
import com.ld.protean.ext.needOpenSELinux
import com.ld.protean.ext.reportDuration
import com.ld.protean.ext.speed
...
import com.ld.protean.ui.viewmodel.SettingsViewModel
...
class SettingsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // 地图源：换源后返回地图即生效（地图 Fragment 重建时读取该配置）
        binding.mapSourceValue.text = MapTiles.provider(context.mapTileProvider).label
        binding.mapSourceLayout.setOnClickListener {
            val providers = MapTiles.providers
            val labels = providers.map { "${it.label} — ${it.description}" }.toTypedArray()
            val checked = providers.indexOfFirst { it.id == context.mapTileProvider }

            MaterialAlertDialogBuilder(context)
                .setTitle("选择地图源")
                .setSingleChoiceItems(labels, checked) { dialog, which ->
                    context.mapTileProvider = providers[which].id
                    binding.mapSourceValue.text = providers[which].label
                    dialog.dismiss()
                    showToast("已切换到${providers[which].label}，返回地图生效")
                }
        ...
        // 反定位拉回：让模块持续重播模拟位置，压回被 App 拉走的真实位置
        binding.loopBroadcastSwitch.isChecked = requireContext().loopBroadcastLocation
        binding.loopBroadcastSwitch.setOnCheckedChangeListener { _, isChecked ->
            requireContext().loopBroadcastLocation = isChecked
            with(mockServiceViewModel) {
                if(!MockServiceHelper.setLoopBroadcastLocation(locationManager!!, isChecked)) {
                    showToast("反定位拉回设置失败: 无法连接到系统服务")
                } else {
