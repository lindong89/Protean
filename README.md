...
- 本应用**不集成任何统计、崩溃上报或广告 SDK**，不会把使用数据或设备标识上传到任何服务器。
- 应用发起的网络请求仅用于地图与地址搜索：
  - 地图瓦片：Esri ArcGIS 在线底图、OpenStreetMap、高德（可在设置里切换图源）
  - 地址搜索与逆地理编码：[Photon](https://photon.komoot.io/)（`photon.komoot.io`）
  - 这些请求会由对应服务方按常规方式记录访问日志（含 IP），与用浏览器访问地图网站类似。
- 定位等权限只在本地使用，模拟结果仅作用于本机系统定位服务。
